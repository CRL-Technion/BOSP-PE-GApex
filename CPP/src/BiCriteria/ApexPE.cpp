...
ApexPESearch::ApexPESearch(const AdjacencyMatrix &adj_matrix, EPS eps, 
...
void ApexPESearch::insert(ApexPathPairPtr &ap, APQueue &queue) {
    last_ap_inserted = false;
...
void ApexPESearch::merge_to_solutions(const ApexPathPairPtr &ap, ApexPathSolutionSet &solutions) {
...
bool ApexPESearch::is_dominated(ApexPathPairPtr ap)
{
    dom_check_counter++; // yaron
    if (local_dom_checker->is_dominated(ap))
    {
        dom_check_local++;
        if(verbose) std::cout << "Local Domination Found\n";
        return true;
    }
    ...
    {
        dom_check_solution++;
        if(verbose) std::cout << "Solution Domination Found\n";
        return true;
    }
...
void ApexPESearch::operator()(size_t source, size_t target, Heuristic &heuristic, SolutionSet &solutions, unsigned int time_limit) 
{
    ...
    while (open.empty() == false) {
        ...
        meas_start = std::chrono::high_resolution_clock::now();
        
        // Generating the next sibling of the current expanded node
        if (ap->parent != nullptr && adj_matrix.is_boundary_vertex(ap->parent->id))
        {
            const std::vector<Edge>& parent_outgoing_super_edges = adj_matrix.get_super_edges(ap->parent->id);
            for (size_t edge_idx = ap->out_super_edge_ind + 1; 
                edge_idx < parent_outgoing_super_edges.size(); 
                edge_idx++)
            {
                // Local domination check
                for (int k = 0; k < num_of_objectives; k++)
                {
                    temp_cost_vec[k] = ap->parent->apex->g[k] + parent_outgoing_super_edges[edge_idx].cost[k];
                }
                if (local_dom_checker->is_dominated_lite(parent_outgoing_super_edges[edge_idx].target,
                    temp_cost_vec))
                {
                    continue;
                }
                // Solution domination check
                temp_h = heuristic(parent_outgoing_super_edges[edge_idx].target);
                for (int k = 0; k < num_of_objectives; k++)
                {
                    temp_cost_vec[k] += temp_h[k];
                }
                if (solution_dom_checker->is_dominated_lite(parent_outgoing_super_edges[edge_idx].target,
                    temp_cost_vec))
                {
                    continue;
                }

                next_ap = std::make_shared<ApexPathPair>(ap->parent, parent_outgoing_super_edges[edge_idx]);
                next_ap->out_super_edge_ind = edge_idx;         
        ...
        const std::vector<Edge>& outgoing_regular_edges = adj_matrix.get_regular_edges(ap->id);
        for (size_t edge_idx = 0; edge_idx < outgoing_regular_edges.size(); edge_idx++)
        {
            // Local domination check
            for (int k = 0; k < num_of_objectives; k++)
            {
                temp_cost_vec[k] = ap->apex->g[k] + outgoing_regular_edges[edge_idx].cost[k];
            }
            if (local_dom_checker->is_dominated_lite(outgoing_regular_edges[edge_idx].target,
                temp_cost_vec))
            {
                continue;
            }
            ...
            // Solution domination check
            temp_h = heuristic(outgoing_regular_edges[edge_idx].target);
            for (int k = 0; k < num_of_objectives; k++)
            {
                temp_cost_vec[k] += temp_h[k];
            }
            if (solution_dom_checker->is_dominated_lite(outgoing_regular_edges[edge_idx].target,
                temp_cost_vec))
            {
                continue;
            }

            next_ap = std::make_shared<ApexPathPair>(ap, outgoing_regular_edges[edge_idx]);
            next_ap->out_regular_edge_ind = edge_idx;
            this->insert(next_ap, open);
        }

        
        // After regular edges were handled, inserting only a single super-edge child
        meas_start = std::chrono::high_resolution_clock::now();
        if (adj_matrix.is_boundary_vertex(ap->id))
        {
            const std::vector<Edge>& outgoing_super_edges = adj_matrix.get_super_edges(ap->id);
            for (size_t edge_idx = 0;
                edge_idx < outgoing_super_edges.size();
                edge_idx++)
            {
                // Local domination check
                for (int k = 0; k < num_of_objectives; k++)
                {
                    temp_cost_vec[k] = ap->apex->g[k] + outgoing_super_edges[edge_idx].cost[k];
                }
                if (local_dom_checker->is_dominated_lite(outgoing_super_edges[edge_idx].target,
                    temp_cost_vec))
                {
                    continue;
                }
                ...
                // Solution domination check
                temp_h = heuristic(outgoing_super_edges[edge_idx].target);
                for (int k = 0; k < num_of_objectives; k++)
                {
                    temp_cost_vec[k] += temp_h[k];
                }
                if (solution_dom_checker->is_dominated_lite(outgoing_super_edges[edge_idx].target,
                    temp_cost_vec))
                {
                    continue;
                }
                
                next_ap = std::make_shared<ApexPathPair>(ap, outgoing_super_edges[edge_idx]);
                next_ap->out_super_edge_ind = edge_idx;
                this->insert(next_ap, open);
                if (last_ap_inserted)
                {
        ...
        auto meas_end1 = std::chrono::high_resolution_clock::now();
        auto duration1 = std::chrono::duration_cast<std::chrono::microseconds>(meas_end1 - meas_start);
        apex_creation_total_time += std::chrono::duration_cast<std::chrono::microseconds>(duration1).count() * 1e-6;
    }
    ...
    std::cout << "Number of heap insertions = " << heap_insert_count << std::endl;
...
std::string ApexPESearch::get_solver_name() 
{
